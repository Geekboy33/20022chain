...
export async function GET() {
  try {
    ...
    console.error('[network] Unavailable:', e);
    return NextResponse.json({
      node: { peers: 0, status: 'offline' },
      chain: { blocks: 0, transactions: 0, validators: 0, isins: 0 },
      network: { protocol: 'WebSocket', version: '1.0.0', consensus: 'Proof-of-Stake', blockTime: '6s', p2pPort: 0 },
    });
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error('[network POST] Unavailable:', e);
